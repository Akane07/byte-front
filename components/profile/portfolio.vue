<template>
    <div
        ...
        <div class="w-full h-full">
            <img class="w-full h-full relative z-1 object-cover" :src="makeURL(portfolio.images[0])" alt="photo">
            <UIDevLoader class="loader absolute top-[50%] left-[50%] z-0" />
        </div>
        <div class="hover absolute top-0 left-0 w-full h-full z-2"
            @click.stop="navigateTo(`/profile/portfolio/${portfolio.id}`)">
            <div class="absolute flex gap-2 top-2.5 right-3.5" v-if="!other">
                <div class="button w-10 h-6.5 flex items-center justify-center rounded-md" @click.stop="handleDelete">
                    <IconsTrash style="transform: scale(1.3);"></IconsTrash>
                </div>
                <div class="button w-10 h-6.5 flex items-center justify-center rounded-md"

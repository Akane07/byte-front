        ...
        <div ref="wrapperRef" class="step_wrapper">
                ...
                <div class="right_part">
                    ...
                    <div class="block">
                        ...
                        <div class="skills_block" @click="handleFocus">
                            ...
                            <input v-model="newSkill" ref="inputRef" type="text" @keyup.enter="handleAddSkill"
                                maxlength="20" />
                        </div>
                    </div>
                    <div class="block">
                    ...
                    <div class="block">
                        ...
                        <div class="custom_checkbox">
                            ...
                            <div @click="newOrder.for_experts = !newOrder.for_experts" class="checkbox">
                                <IconsBluePencil v-if="newOrder.for_experts" style="transform: scale(1.5);">
                                </IconsBluePencil>
                            </div>
                        </div>
                        <div class="custom_checkbox">
                            ...
                            <div @click="newOrder.type === 'one-time' ? (newOrder.type = 'reusable') : (newOrder.type = 'one-time')"
                                class="checkbox">
                                <IconsBluePencil v-if="newOrder.type === 'reusable'" style="transform: scale(1.5);">
                                </IconsBluePencil>
                            </div>
                        </div>
                    </div>
            ...
            <div class="step_block fourth">
                ...
                <div class="right_part">
                    ...
                    <span class="tip">Установите цену за заказ и заплатите в конце, или вы можете разделить заказ на
                        этапы и
                        платить по мере выполнения каждого этапа.</span>
                    <div class="input_block"
                        v-if="(newOrder.price_type === 'fixed') || (newOrder.price_type === 'contract')">
...
onMounted(async () => {
    ...
    const order = await orderStore.getOrder(route.params.id as string);

    if (!order) return;

    await categoryStore.getAllCategories();

    newOrder.value.category = categoryStore.getCategoryTitleById(order.category);
    newOrder.value.deadline_date = order.deadline_date;
    newOrder.value.deadlines = order.deadlines;
    newOrder.value.description = order.description;
    newOrder.value.draft = order.draft;
    newOrder.value.for_experts = order.for_experts;
    newOrder.value.price = order.price;
    newOrder.value.price_type = order.price_type;
    newOrder.value.skills = order.skills;
    newOrder.value.title = order.title;
    newOrder.value.type = order.type;
    id.value = order.id;

    await categoryStore.getSkillsById(newOrder.value.category);

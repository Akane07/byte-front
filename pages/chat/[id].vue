<template>
  <div class="chat" v-if="renderChat">
    <div class="head">
      <div class="info" v-if="user">
        ...
        <div class="name">
          <p @click="navigateTo(`/profile/${user.id}`)" style="cursor: pointer">
            {{ user?.name }}
          </p>
          <div class="online">
      ...
      <div class="buttons" v-if="user">
        ...
        <button
          class="request"
          @click="navigateTo(`/orders/suggest?id=${user.id}`)"
        >
      ...
      <template v-if="user">
        <div
          ...
          :class="[
            ...
            msg.status === 'server' ? 'server' : '',
          ]"
        >
          <div
            v-if="
              !msg.is_suggest && msg.status !== 'response' && !msg.responseId
            "
            class="message"
          >
            <p>{{ msg.text }}</p>
            <img
              v-if="msg.mediaType === 'image' && msg.status !== 'server'"
              :src="msg.mediaUrl"
              width="200"
            />
            <video
              v-if="msg.mediaType === 'video' && msg.status !== 'server'"
              :src="msg.mediaUrl"
              width="200"
              controls
            />
            <span class="time" v-if="msg.status !== 'server'">{{
              useMessageCreated(msg.createdAt)
            }}</span>
          </div>
          <div
            class="response"
            v-else-if="
              msg.status === 'response' && msg.senderId === userStore.user?.id
            "
          >
            ...
            <div class="deadlines">
              <p>
                ...
                <span>{{
                  useOrderDeadlines(
                    ordersInChat.find((order) => order.id === msg.orderId)
                      ?.deadlines,
                    ordersInChat.find((order) => order.id === msg.orderId)
                      ?.deadline_date
                  )
            ...
            <div
              class="buttons"
              v-if="msg.status !== 'rejected' && msg.status !== 'accepted'"
            >
              <button
                class="delete"
                @click="
                  handleDeleteResponse(msg.responseId, msg.orderId, msg.id)
                "
              >
                Удалить отклик
              </button>
          ...
          <div class="response" v-else-if="msg.status === 'response'">
            ...
            <div
              ...
              v-if="msg.status !== 'rejected' && msg.status !== 'accepted'"
            >
              <button class="view" @click="openModal(msg.orderId, msg.id)">
                Принять
              </button>
              <button class="delete" @click="rejectSuggest(msg.id)">
                Отклонить
              </button>
            </div>
          </div>
          <div
            class="response"
            v-else-if="msg.status !== 'response' && msg.responseId"
          >
            <!-- Предложение заказа от получателя -->
            <div class="header">
              <IconsSuggest></IconsSuggest>
          ...
          <div
            v-else-if="msg.is_suggest && msg.senderId === userStore.user?.id"
            class="suggest"
          >
            <!-- Предложение заказа от отправителя -->
            <div class="header">
            ...
            <div
              ...
              v-if="msg.status !== 'rejected' && msg.status !== 'accepted'"
            >
              <button class="delete" @click="deleteSuggest(msg.id)">
                Отозвать
              </button>
            </div>
          </div>
  ...
  <UIDevCustomModal v-if="modalAccept && order" @close="modalAccept = false">
    <div class="left_part">
      ...
      <div class="buttons">
        <button class="view" @click="acceptSuggest(currentMessageId)">
          Принять
        </button>
        <button class="delete" @click="rejectSuggest(currentMessageId)">
          Отклонить
        </button>
      </div>
    </div>
